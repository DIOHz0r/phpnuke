...
class database
{
     ...
     * @param bool If true and there is an error, go offline
     */
    ...
    /**
     * @param bool If TRUE, displays the last SQL statement sent to the database
     *
...
class mosDBTable
{
     ...
     *	can be overloaded/supplemented by the child class
     *
     *	@return bool True if the object is ok
     */
     ...
     * Can be overloaded/supplemented by the child class
     *
     * @param bool If false, null object variables are not updated
     *
...
//echo 'D: ' . $this->_db->getQuery();
    ...
    public function updateOrder($where = '')
    {
        ...
        for ($i = 0; $i < $n; ++$i) {
            ...
            if ($orders[$i]->$k == $this->$k) {
                ...
                $shift = 1;
            } elseif ($orders[$i]->ordering >= $this->ordering && $this->ordering > 0) {
                ++$orders[$i]->ordering;
            }
        }
     ...
     * @param int 0 if unpublishing, 1 if publishing
     * @param int The id of the user performnig the operation
     *
     * @since 1.0.4
     */
